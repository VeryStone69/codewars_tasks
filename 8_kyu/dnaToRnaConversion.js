...
function DNAtoRNA(dna) {
    return dna.split("").map(el=>el.replace(/T/g,"U")).join("")
}

function runTests() {
    const tests = [
        { input: "ATTGCT", expected: "AUUGCU" },
        { input: "TTTT", expected: "UUUU" },
        { input: "GCGCGC", expected: "GCGCGC" },
        { input: "", expected: "" },
        { input: "TACGTA", expected: "UACGUA" }
    ];

    tests.forEach((test, index) => {
        const result = DNAtoRNA(test.input);
